            ...
            SwitchableValue.ON -> {
                desiredState.isVolumeMuted?.let { isMuted ->
                    DefaultResolvedServiceCommand(
                        ...
                        serviceData = TelevisionDesiredServiceData(isVolumeMuted = isMuted)
                    )
                } ?: desiredState.volumeLevel?.let { volumeLevel ->
                    DefaultResolvedServiceCommand(
                        service = TelevisionService.VOLUME_SET,
                        serviceData = TelevisionDesiredServiceData(volumeLevel = volumeLevel)
                    )
                } ?: desiredState.source?.let { source ->
                    DefaultResolvedServiceCommand(
                        service = TelevisionService.SELECT_SOURCE,
                        serviceData = TelevisionDesiredServiceData(source = source)
                    )
                } ?: DefaultResolvedServiceCommand(
